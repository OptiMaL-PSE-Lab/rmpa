...
def run_iron_and_steel_ellipse(u,epsilon,g):
    ...
    p = {}
    p["capture_rate"] = {"val": 0.63, "unc": 0.63*u}
    p["initial_captured"] = {"val": 10000000, "unc":10000000*u }
    p["learning_rate"] = {"val": 0.18, "unc": 0.18*u}
    p["initial_capture_cost"] = {"val": 55.4085, "unc": 55.4085*u}
    for i in range(plants):
        p[plant_names[i] + ": CO2 Emissions"] = {
            "val": plant_emissions[i],
            "unc": plant_emissions[i]*u,
        }
...
def run_iron_and_steel_ellipse_change(u,epsilon,g,ms_max):
    logging.getLogger("pyomo.core").setLevel(logging.ERROR)
    os.system("clear")
    ...
    def minimum_improvement(x, p):
        CR, UC0, CO2, A0, LR = parse_params(p)
        carbon_tax, market_share, t = parse_vars(x)
        return ms_max - (1 - ((A0 + CR * sum(CO2) * sum(market_share)) / A0) ** (-LR))
...
ms_list = np.linspace(0,0.5,21)
overall_percentage = 0.05
tol = 1e-6
prob = 0.01
gamma = np.sqrt(-2*np.log(prob/100))
for ms in ms_list:
    res = run_iron_and_steel_ellipse_change(overall_percentage,1e-6, gamma,ms)
    sol = list(res['robust_solution'].values())[1:]
    incentive = sol[:int(len(sol)/2)]
    market_share = sol[int(len(sol)/2):]
    df = {'carbon tax':incentive,'market share':market_share}
    df = pd.DataFrame(df)
    df.to_csv('outputs/iron_res_'+str(ms)+'.csv')