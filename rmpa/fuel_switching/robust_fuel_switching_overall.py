    ...
    # for it in range(1):
        ...
        for i in range(len(res)):
            if len(res[i]) > 1:
                ...
                con = con_list[i]
                m_upper.cons.add(expr=con(x_vars, res[i][1]) <= 0)
        print('Max constraint violation: ',mcv)
        pool.close()
...
#     rob[0] = 0 
#     for j in range(1,n):
#         # res,sp_av,m,t_nom = run_fuel_switching_ellipse(percentages[i], 1e-4, gamma[j])
#         # with open('outputs/pickle_files/'+str(i)+'_'+str(j)+'.pickle','wb') as f:
#         #     pickle.dump(res,f)
#         with open('outputs/pickle_files/'+str(i)+'_'+str(j)+'.pickle','rb') as f:
#             res = pickle.load(f)
...
# axs.set_xscale("log")
# axs.legend()
# fig.savefig("outputs/ellipse_fuel_switching_all.pdf")
